...
import { useInterviewSessions } from "@/lib/hooks";
...
export default function InterviewHistoryPage() {
  ...
  const {
    ...
    next,
    previous,
  } = useInterviewSessions({ userId: user?.uid });

  const noInterviewSessions = interviewSessions?.length === 0;
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Error State */}
      {interviewSessionsError && (
        <ErrorAlert errorMessage="Failed to load interviews" />
      )}

      {/* Empty State */}
      {!interviewSessionsError && interviewSessions.length === 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
