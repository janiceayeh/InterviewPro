...
export default function SettingsPage() {
  ...
  const [newAdminRole, setNewAdminRole] = useState<'super-admin' | 'content-manager' | 'moderator'>('content-manager')
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      try {
        ...
        setAdmins([
          { id: '1', email: 'admin@interviewpro.com', role: 'super-admin', createdAt: '2024-01-15' },
          { id: '2', email: 'content@interviewpro.com', role: 'content-manager', createdAt: '2024-02-20' },
          { id: '3', email: 'moderator@interviewpro.com', role: 'moderator', createdAt: '2024-03-10' },
        ])
      } catch (error) {
      ...
      <Card className="p-6 border-border/50">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Badge className={roleColor[role as keyof typeof roleColor]} className="mt-1">
              {role?.replace('-', ' ')}
            </Badge>
          </div>
          <div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/30">
                <th className="text-left text-sm font-semibold text-foreground py-3">Email</th>
                <th className="text-left text-sm font-semibold text-foreground py-3">Role</th>
                <th className="text-left text-sm font-semibold text-foreground py-3">Added</th>
                {role === 'super-admin' && <th className="text-right text-sm font-semibold text-foreground py-3">Actions</th>}
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={role === 'super-admin' ? 4 : 3} className="text-center py-8">
                ...
                admins.map((admin) => (
                  <tr key={admin.id} className="border-b border-border/30 hover:bg-muted/50">
                    <td className="py-3 text-foreground font-medium">{admin.email}</td>
                    <td className="py-3">
                      <Badge className={roleColor[admin.role]}>
                        {admin.role.replace('-', ' ')}
                      </Badge>
                    </td>
                    <td className="py-3 text-muted-foreground text-sm">{admin.createdAt}</td>
                    {role === 'super-admin' && (
      ...
      <Card className="p-6 border-border/50">
        ...
        <div className="space-y-3">
          {Object.entries(roleDescriptions).map(([roleName, description]) => (
            <div key={roleName} className="flex items-start gap-4 p-4 bg-muted/30 rounded-lg border border-border/30">
              <div className="flex-1">
                <p className="font-semibold text-foreground capitalize">{roleName.replace('-', ' ')}</p>
